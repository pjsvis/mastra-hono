# pitch for Greg Smith

Nate B Jones : https://www.youtube.com/watch?v=lbfoNxoHl2o&list=TLPQMTIwMzIwMjYZVqWBV7D-BQ&index=10


## Introduction

Greg Smith was my boss when I worked at FIS developing the business logic and UI for a credit card testing (CCT) platform.

The project was successful and is still producing income for the company today.

I was out for dinner with Greg on Friday evening and he described how he was interested in addressing some scaling issues in the CCT system. Additionally I described how I had been working on AI related projects over the past year.

When we parted I suggested that I would put together a pitch for Greg covering the above two issues: scaling CCT and applying AI in FIS, Edinburgh office.

Below are my ideas

## Scaling CCT

1. Immediate Database Optimization (The "Caw Canny" Approach): Transitioning XML storage to SQL Server FILESTREAM.

The Win: Offloads XML data to the NT system cache, bypassing the SQL Server buffer pool. This significantly reduces memory pressure without a total rewrite.

2. Replace front end framework with server side rendering (SSR): 
 
The Win: Server-Side Rendering (SSR) simplifies the stack by serving reactive HTML directly, reducing the maintenance burden of complex client-side frameworks/states.

It is likely the FILESTREAM caching will be sufficient to handle scaling needs, but exploring server side rendering options is worth considering. An AI-assisted coding exercise for the server side rendering would be quick to produce results.


## AI in Edinburgh at FIS

After London, Edinburgh is the largest AI centre in Europe. It makes sense for companies based in Edinburgh to maintain an AI skunkworks to keep an eye on AI developments and applications.

Some ideas for AI development are:

1. Establish an ontology for the FIS domains of interest including relevant legislation and regulations. 
 
The Win: A lexicon of principles, terms, and heuristics that can be used to mitigate the "tendencies" of AI substrates to produce poor results. See the **Bestiary of Substrate Tendencies** below for examples of observed AI failure modes.

2. One technique in AI development is to get the AI to write a program to solve a problem rather than just generating a response. The program can be tested and acts as a deterministic artifact-as-proof part of the AI data processing pipeline.

The Win: Confining the stochastic behavior of AI systems to well defined boundaries leads to more predictable results.

3. The AI development scene is currently in the Cambrian explosion phase. In order to navigate this phase developers need to have a degree of engagement with the technology and a clear understanding of the FIS domains and legal requirements.

## **A Bestiary of Substrate Tendencies**

Below is a lexicon of observed, native behaviors and failure
...
It could be argued that there is a lot of overlap between AI coding agent tendencies and junior developer tendencies.
...
- Nate B Jones is a YouTuber worth watching: https://www.youtube.com/watch?v=iY7BDpZWJbE

- Have a look at the PolyVis website https://polyvis.net/ referred to in the above.
  - The graph explorer is a client-side only hybrid graph-vector database implemented in sqlite and WASM.
  - But the real fun is in 09. Performance Review...

- Give Google's Gemini a try: https://gemini.google.com/gem/14zihcz6UmRpSBf1rh4BmVhZxeNwqWISI?usp=sharing
  - The link above uses the CTX system-prompt to guide the model's reasoning.
  - You should be able to access it if you have a Google email
  - A good first prompt with any LLM is "State your designation and capabilities"
  - That should get a list of what the thing can do.
  - Most of the Chinese models respond with designation Claude Code, because they have had a lot of their stuff "distilled" from Anthropic's models
 
And finally, instead of thinking about "Agentic Workflows...", think first about "Muppets with Cleavers..."

And lastly the POLYVIS About page is worth an hover/click https://polyvis.net/about/
