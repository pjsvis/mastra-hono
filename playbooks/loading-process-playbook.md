...
ast-grep -p '## Section Name' -A 50 playbooks/example-playbook.md

# Extract all code blocks in a section
ast-grep -p '```bash' -A 10 playbooks/example-playbook.md

# Extract a table
ast-grep -p '| Column 1 | Column 2' -A 20 playbooks/example-playbook.md

# Extract all checklists
ast-grep -p '## Checklist' -A 30 playbooks/example-playbook.md
```
...
**Simple grep:**
```bash
grep -A 30 "## Section Name" playbooks/example-playbook.md
```

**sed:**
```bash
sed -n '/## Section Name/,/^## /p' playbooks/example-playbook.md | head -n -1
```

**awk:**
```bash
awk '/## Section Name/,/^## [^#]/ {print}' playbooks/example-playbook.md | head -n -1
```
...
Include the two‑step loading process description (copy from this playbook and customize):
